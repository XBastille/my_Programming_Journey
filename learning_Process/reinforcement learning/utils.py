...
def create_video(filename, env, q_network, fps=30):
    """
    Creates a video of an agent interacting with a Gym environment using OpenCV.

    Args:
        filename (string): The path to the file to which the video will be saved.
        env (Gym Environment): The Gym environment the agent will interact with.
        q_network (tf.keras.Sequential): A TensorFlow Keras Sequential model that maps states to Q values.
        fps (int): The number of frames per second. Specifies the frame rate of the output video.
    """
    import cv2
    ...
    state = env.reset()[0]
    while not done:
        # Render the frame and convert it from RGB (default by Gym) to BGR (default by OpenCV)
