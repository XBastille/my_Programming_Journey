...
class Json(sdict, dict, MutableMapping[K, V]):
    ...
    def __setattr__(self, key, value):
        if key not in self.__myAttrs():
            ...
            raise AttributeError(
                f"'{str(self.__class__)}' object attribute '{key}' is read-only"
            )
    ...
    def __delattr__(self, name):
        if name in self:
            ...
            raise AttributeError(f"{str(self.__class__)} instance has no attribute {name}")
...
class FrozenJson(Json):
    ...
    def __setattr__(self, key, value):
        raise TypeError(
            f"Can not update a FrozenJson instance by (key,value): ({key},{value})"
        )
