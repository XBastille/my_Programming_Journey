...
class ExtModel(BaseModel):
    """
    Extended Model with supercomputer's SMP.
    Extends the standard Pydantic model functionality by allowing arbitrary types and providing custom encoding.

    The main purpose of existing this class is to fix encoding issue in PyDantic and provide properly working supercomputer's SMP for arbitrary types.
...
class JsonData(ABC):
    ...
    Dataclass with supercomputer's SMP.
    ...
    def _get_combined_encoders(cls):
        ...
        for base in reversed(cls.__mro__):
            if hasattr(base, "Config") and hasattr(base.Config, "json_encoders"):
                combined_encoders |= base.Config.json_encoders
